          ...
          <div className="p-6 md:p-8 space-y-6">
            <div>
              <div className="flex items-center gap-3 mb-3">
                ...
                <span className="text-xs text-[rgba(255,255,255,0.3)]" style={{ fontFamily: "'DM Mono', monospace" }}>{p.year}</span>
                <span className="text-xs text-[rgba(255,255,255,0.2)]" style={{ fontFamily: "'DM Mono', monospace" }}>{p.num}</span>
              </div>
              <h2 className="text-3xl font-black text-gradient mb-3" style={{ fontFamily: "'Syne', sans-serif" }}>{p.title}</h2>
              <p className="text-sm leading-relaxed text-[rgba(255,255,255,0.5)]">{p.longDescription || p.description}</p>
            </div>
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                {p.tags.map(t => (
                  <span
                    key={t}
                    className="rounded-xl border border-[rgba(0,255,135,0.12)] bg-[rgba(0,255,135,0.04)] px-3 py-1.5 text-xs font-medium text-[rgba(0,255,135,0.5)] hover:border-[rgba(0,255,135,0.3)] hover:text-[#00FF87] transition-colors"
  ...
  >
    ...
    <div className="relative z-10 flex flex-col md:flex-row gap-0">
      ...
      <div className="flex-1 p-8">
        ...
        <p className="mb-5 text-sm leading-relaxed text-[rgba(255,255,255,0.4)]">{p.description}</p>
        <div className="mb-6 flex flex-wrap gap-2">
          {p.tags.map(t => (
            <span key={t} className="rounded-xl border border-[rgba(0,255,135,0.1)] px-3 py-1 text-xs text-[rgba(0,255,135,0.4)]" style={{ fontFamily: "'DM Mono', monospace" }}>{t}</span>
          ))}
        </div>
  ...
  >
    ...
    <div className="relative z-10">
      <div className="relative h-40 overflow-hidden">
        ...
        <span className="absolute bottom-3 left-4 text-xs text-[rgba(255,255,255,0.3)]" style={{ fontFamily: "'DM Mono', monospace" }}>{p.num}</span>
      </div>
      <div className="p-6">
        <div className="mb-2 flex items-center justify-between">
          <span className="text-xs text-[rgba(255,255,255,0.25)]" style={{ fontFamily: "'DM Mono', monospace" }}>{p.year}</span>
        ...
        <p className="mb-4 text-xs leading-relaxed text-[rgba(255,255,255,0.35)] line-clamp-2">{p.description}</p>
        <div className="flex flex-wrap gap-1.5">
          {p.tags.slice(0, 3).map(t => (
            <span key={t} className="rounded-xl border border-[rgba(0,255,135,0.1)] px-2.5 py-1 text-[10px] text-[rgba(0,255,135,0.4)]" style={{ fontFamily: "'DM Mono', monospace" }}>{t}</span>
          ))}
        </div>
      </div>
    </div>
        ...
        <div className="mx-auto max-w-6xl">
          ...
          >
            <a
              ...
              className="inline-flex items-center gap-2 text-sm text-[rgba(255,255,255,0.3)] hover:text-[#00FF87] transition-colors link-anim"
              style={{ fontFamily: "'DM Mono', monospace" }}
            >
